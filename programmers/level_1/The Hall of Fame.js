...
function solution(k, score) {
  ...
  for (let i = 0; i < score.length; i++) {
    ...
    // 내림차순으로 정렬
    _list.sort((a, b) => b - a);
    //i값이 k보다 작을 시에는 가장 마지막 값 _list에 저장
    //i값이 k보다 클 시에는 k - 1의 값 저장
    if (i < k) {
      last_number = _list.slice(-1);
    } else {
      last_number = _list.slice(k - 1);
    }
    //각각의 배열로 나눠져 있는 것들을 풀어서 하나로 저장
    answer.push(parseInt(last_number.join()));
  }
