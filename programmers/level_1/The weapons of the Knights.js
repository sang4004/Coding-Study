...
function solution(number, limit, power) {
  ...
  for(let i = 1; i <= number; i++){
      // 반으로 나누기 때문에 자기 자신 값 + 1
      let sum = 1;
      // 1 부터 현재 수의 절반까지 반복문을 돌림
      // 약수는 본래 값을 제외하고는 n/2보다 클 수 없기 때문
      for(let j = 1; j <= i/2; j++){
        // 나머지가 0이면 약수
          if(i % j === 0){
            //약수 일 경우 sum에 + 1
      ...
      // sum 값이 limit 보다 크지 않을 경우 sum 값을 더하고
      // sum 값이 limit 보다 클 경우 power 값을 더함
      answer += limit >= sum ? sum : power;
  }
  return answer;
}